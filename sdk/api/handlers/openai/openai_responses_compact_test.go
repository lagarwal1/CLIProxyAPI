...
func TestOpenAIResponsesStripsPromptCacheRetention(t *testing.T) {
	gin.SetMode(gin.TestMode)
	executor := &compactCaptureExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)
	...
	reqBody := `{"model":"test-model","input":"hello","prompt_cache_retention":"24h","prompt_cache_key":"cache-key"}`
	...
	if gjson.GetBytes(executor.payload, "prompt_cache_retention").Exists() {
		t.Fatalf("forwarded payload still contains prompt_cache_retention: %s", string(executor.payload))
	}
	if got := gjson.GetBytes(executor.payload, "prompt_cache_key").String(); got != "cache-key" {
